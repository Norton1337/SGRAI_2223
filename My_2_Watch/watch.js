...
export default class Watch extends THREE.Group {
    constructor(cityName, center = new THREE.Vector2(0.0, 0.0), radius = 0.75, nameBackgroundColor = 0xffffff, nameForegroundColor = 0x000000, dialColor = 0x000000, markersColor = 0xffffff, handsHMColor = 0xffffff, handSColor = 0xff0000) {
        ...
        */
        ...
        let divisions60 = 60;
        let xStart = 0;
        const xIncrement = Math.PI/divisions60;
        let yStart = 0;
        const yIncrement = Math.PI/divisions60;
        
        for (let i = 0; i < divisions60*2; i++) {
            let x = radius * Math.cos(xStart) ;
            let y = radius * Math.sin(yStart) ;  
        ...
        points.push(new THREE.Vector3(0,radius/2,0));
        geometry = new THREE.BufferGeometry().setFromPoints(points);
        ...
        points = [];
        points.push(new THREE.Vector3(0,0,0));
        points.push(new THREE.Vector3(0,radius*0.7,0));
        geometry = new THREE.BufferGeometry().setFromPoints(points);
